...
public class AutoCenter8 extends SequentialCommandGroup {
	...
	public AutoCenter8(ChassisSubsystem chassisSubsystem, IntakeSubsystem intakeSubsystem, ShooterSubsystem shooterSubsystem) throws IOException {
		super(
			new AutoFollowPath(chassisSubsystem, "output/center8-1.wpilib.json"),
			...
			new AutoFollowPath(chassisSubsystem, "output/center8-2.wpilib.json"),
			new WaitCommand(Constants.AUTO_INTAKE_MIN_DURATION),
			new IntakeRollersSetAuto(intakeSubsystem, 0),
			new AutoFollowPath(chassisSubsystem, "output/center8-3.wpilib.json"),
			new ShooterShoot(shooterSubsystem, Constants.AUTO_SHOOT_SETPOINT),
			new WaitCommand(Constants.AUTO_SHOOT_DURATION),
            new ShooterStop(shooterSubsystem)
		);
